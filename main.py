...
class Anime_Info(ft.Container):
    def __init__(self, page, back):
        ...
        self.themes_text = ft.Container(**self.box_text_style(type="ThÃ¨mes"))
        ...
        for attempt in range(max_retries):
            try:
                ...
                if response.status_code == 404:
                    # Si l'ID de l'anime est incorrect, arrÃªter les tentatives
                    break
            except requests.exceptions.ConnectionError:
                print("Erreur de connexion. Nouvelle tentative...")
            except requests.exceptions.Timeout:
                print("DÃ©lai d'attente dÃ©passÃ©. Nouvelle tentative...")
            except requests.exceptions.RequestException as err:
                print(f"Erreur de requÃªte : {err}")
                # Pour toute autre erreur de requÃªte, arrÃªter les tentatives
                break
            
            # Attendre avant de refaire une tentative
            time.sleep(retry_delay)
            retry_delay += 2  # Augmenter le dÃ©lai entre les tentatives
    
    def add_to_list(self, e):
        name = self.name.content.controls[0].value
        image_src = self.image.src
        dossier = "/storage/emulated/0/Dossier"    #r"C:\Users\Abdoul Karim\Desktop\Nouveau dossier\scripts\Dossier"
        ...
        if response.status_code == 200:
            ...
            self.notif.content.value = "Anime ajouter Ã  la watchlist"
            self.notif.open = True
            self.page.update()
        else:
            self.notif.content.value = "Veuillez patienter et rÃ©essayer"
            self.notif.open = True
            self.page.update()
...
class Anime_Results(ft.Container):
    ...
    def correspond_titre(self, anime, mots_cles):
        ...
        # Nettoyer chaque titre et vÃ©rifier la correspondance
        for titre in titres:
    ...
    def get_anime_results(self, nom_anime):
        ...
        if response.status_code == 200:
            ...
            # Si des rÃ©sultats ont Ã©tÃ© trouvÃ©s
            if data['data']:
                ...
                if animes_filtres:
                    # Trier les animes filtrÃ©s par ordre croissant de leur ID
                    animes_filtres.sort(key=lambda anime: anime['mal_id'])
                    
                    # Retourner les informations des animes triÃ©s
                    return [
                        {
                            "id": anime['mal_id'],
                            "title": anime['title'],
                    ...
                    return "Aucun anime correspondant trouvÃ©."
            else:
                return "Aucun anime trouvÃ© pour ce nom."
        else:
...
def main(page: ft.Page):
    ...
    page.add(ft.SafeArea(content=search_page))
